...
class Filter {
    ...
    public filterCards(data: Array<cardItem>): void {
        ...
        if (Search.search?.value !== '' && Search.search?.value !== undefined) {
        ...
        Sort.sortSelect && (Sort.sortSelect.value = localStorage.getItem('sortOption') || Sort.sortSelect?.value);
